...
def _combined_centered_product(
    ...
    Coordinates must be finite; callers select representable samples beforehand.
    """
    coordinate_scale = be.maximum(be.abs(x), be.abs(y))
    ...
    x_normalized = x / safe_coordinate_scale - x_ref / safe_coordinate_scale
    y_normalized = y / safe_coordinate_scale - y_ref / safe_coordinate_scale
    ...
    with be.errstate(over="ignore", invalid="ignore"):
        return (combined * smaller_scale) * larger_scale
