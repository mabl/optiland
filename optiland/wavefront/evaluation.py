...
def _weighted_mean_parts(
    ...
    mean = _add_terms([anchor, correction])
    # A nonfinite mean also makes every centered difference nonfinite, so the
    # refinement guard below covers it without a separate check.
...
def _weighted_average(
    ...
    *,
    ...
    parts = (values,) if error is None else (values, error)

    def weighted_terms(value_scale: BEArrayT | float | None) -> BEArrayT:
        """Weight every value part, optionally after a common value rescaling."""
        scaled = parts if value_scale is None else [p / value_scale for p in parts]
        if lost_weight:
            terms = [(sqrt_alpha * part) * sqrt_alpha for part in scaled]
        else:
            terms = [scaled_weights * part for part in scaled]
        return terms[0] if len(terms) == 1 else be.concatenate(terms)

    denominator = _sum_samples(
        sqrt_alpha * sqrt_alpha if lost_weight else scaled_weights
    )
    numerator_terms = weighted_terms(None)
    ...
    if be.all(be.isfinite(correction)):
        return correction
    value_scale = max(be.max(be.abs(part)) for part in parts)
    correction = _sum_samples(weighted_terms(value_scale)) / denominator
    with be.errstate(over="ignore", invalid="ignore"):
