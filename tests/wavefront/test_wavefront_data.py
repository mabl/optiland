...
def test_native_intensity_must_use_the_active_backend(set_test_backend):
    data = _wavefront_data()
    if be.get_backend() == "numpy":
        torch = pytest.importorskip("torch")
        data.intensity = torch.ones(5, dtype=torch.float64)
    else:
        data.intensity = np.ones(5, dtype=np.float64)

    with pytest.raises(TypeError, match=f"active {be.get_backend()} backend"):
        data.evaluate(remove="none")


def test_native_intensity_must_share_the_opd_device(set_test_backend):
    """Device validation precedes evaluation, so metadata-only tensors suffice."""
    if be.get_backend() != "torch":
        pytest.skip("Device validation requires the Torch backend.")
    torch = pytest.importorskip("torch")
    data = _wavefront_data()
    data.intensity = torch.ones(5, dtype=torch.float64, device="meta")

    with pytest.raises(TypeError, match="same device as opd"):
        data.evaluate(remove="none")
