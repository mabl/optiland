...
from optiland.wavefront._evaluation_math import (
    _stable_norm,
...
@pytest.mark.parametrize("dtype_name", ["float32", "float64"])
def test_stable_norm_of_zero_vector_is_zero(set_test_backend, dtype_name):
    """The scaled norm kernel must not divide a zero vector by its own scale."""
    result = _stable_norm(_typed_array([0.0, 0.0, 0.0], dtype_name))

    assert_allclose(result, 0.0, rtol=0.0, atol=0.0)
    assert str(result.dtype).endswith(dtype_name)


@pytest.mark.parametrize("invalid_value", [float("nan"), float("inf")])
def test_sample_reduction_passes_nonfinite_operands_through(
    set_test_backend, invalid_value
):
    """Nonfinite operands keep the ordinary reduction; callers decide the policy."""
    result = _sum_samples_forward(_array([1.0, -1.0, invalid_value]))

    assert not be.all(be.isfinite(result))


def test_weighted_scale_rejects_nonfinite_spread(set_test_backend):
    """An overflowing weighted spread cannot scale the affine design."""
    infinite = float("inf")

    with pytest.raises(ValueError, match="no finite positive-weight spread"):
        _weighted_scale(_array([infinite, -infinite]), "x")
...
@pytest.mark.parametrize("invalid_value", [float("nan"), float("inf")])
@pytest.mark.parametrize("remove", ["none", "piston"])
def test_implicit_equal_weights_reject_invalid_opd(
    set_test_backend, invalid_value, remove
):
    """Equal-sample support covers every sample, so invalid OPD must fail."""
    with pytest.raises(ValueError, match="opd_waves must be finite"):
        evaluate_wavefront(_array([0.0, 1.0, invalid_value]), remove=remove)


@pytest.mark.parametrize("input_name", ["opd_waves", "weights", "x", "y"])
@pytest.mark.parametrize("values", [["a", "b"], [object(), object()]])
def test_non_numeric_sequences_are_rejected(set_test_backend, input_name, values):
    kwargs = {"opd_waves": _array([0.0, 1.0])}
    kwargs[input_name] = values

    with pytest.raises(TypeError, match=rf"{input_name} must be a real floating"):
        evaluate_wavefront(remove="piston", **kwargs)


@pytest.mark.parametrize("input_name", ["weights", "x", "y"])
def test_foreign_device_inputs_are_rejected(set_test_backend, input_name):
    """Device validation precedes arithmetic, so metadata-only tensors suffice."""
    if be.get_backend() != "torch":
        pytest.skip("Device validation requires the Torch backend.")
    torch = pytest.importorskip("torch")
    kwargs = {input_name: torch.ones(2, dtype=torch.float64, device="meta")}

    with pytest.raises(TypeError, match=rf"{input_name} must be on the same device"):
        evaluate_wavefront(
            _typed_array([0.0, 1.0], "float64"), remove="piston", **kwargs
        )
...
def test_nonfinite_singular_values_are_numerical_error(set_test_backend, monkeypatch):
    """A backend may return nonfinite factors instead of raising."""
    original_svd = be.linalg.svd

    def nonfinite_svd(*args, **kwargs):
        u_matrix, singular_values, vh_matrix = original_svd(*args, **kwargs)
        return u_matrix, singular_values * be.array(float("nan")), vh_matrix

    monkeypatch.setattr(be.linalg, "svd", nonfinite_svd)

    with pytest.raises(
        WavefrontEvaluationNumericalError, match="nonfinite singular values"
    ):
        evaluate_wavefront(
            _array([0.0, 1.0, 2.0, 4.0]),
            x=_array([0.0, 1.0, 0.0, 1.0]),
...
@pytest.mark.parametrize("dtype_name", ["float32", "float64"])
def test_unrepresentable_affine_coefficients_are_numerical_failure(
    set_test_backend, dtype_name
):
    """An ill-conditioned design with extreme OPD overflows the solved slopes."""
    maximum = np.finfo(getattr(np, dtype_name)).max
    x = np.array([-2.0, -1.0, 0.0, 1.0, 2.0, 3.0])
    step = 1e-6 if dtype_name == "float32" else 1e-12

    with pytest.raises(WavefrontEvaluationNumericalError, match="nonfinite coeff"):
        evaluate_wavefront(
            _typed_array([0.0, maximum, -maximum, maximum, -maximum, 0.0], dtype_name),
            x=_typed_array(x, dtype_name),
            y=_typed_array(
                x + np.array([0.0, step, step, -step, -step, 0.0]), dtype_name
            ),
            remove="piston_tilt",
            rcond=0.0,
        )
...
@pytest.mark.parametrize("dtype_name", ["float32", "float64"])
def test_unrepresentable_affine_residual_is_numerical_failure(
    set_test_backend, dtype_name
):
    """A near-zero weight cannot restore a residual beyond the dtype range."""
    dtype = getattr(np, dtype_name)
    maximum = np.finfo(dtype).max
    smallest = np.nextafter(dtype(0.0), dtype(1.0), dtype=dtype)

    with pytest.raises(WavefrontEvaluationNumericalError, match="affine residual"):
        evaluate_wavefront(
            _typed_array(
                [0.0, 0.9 * maximum, 0.0, 0.9 * maximum, -0.9 * maximum], dtype_name
            ),
            x=_typed_array([0.0, 1.0, 0.0, 1.0, 2.0], dtype_name),
            y=_typed_array([0.0, 0.0, 1.0, 1.0, 2.0], dtype_name),
            weights=_typed_array([1.0, 1.0, 1.0, 1.0, smallest], dtype_name),
            remove="piston_tilt",
        )
